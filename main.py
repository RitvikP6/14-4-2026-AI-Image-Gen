...
def _safe_hf_token() -> str:
    """Best-effort token lookup that never raises."""
    getter = getattr(config, "get_hf_api_key", None)
    if callable(getter):
        try:
...
def gen_image(prompt: str):
    ...
    try:
        ...
        if any(x in msg for x in ["401", "Unauthorized", "Invalid username or password"]):
            token_present = bool(_safe_hf_token())
            return None, (
